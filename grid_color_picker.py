...
while True:
    ...
    if hands:
        ...
        if len(lmList) > 12:
            ...
            thumb_tip = lmList[4]  # thumb tip landmark for color picking

            # Check for color picker activation (thumb on rectangle)
            if not show_palette:
                for rect in rectList:
                    if (rect.posCenter[0] - rect.size[0] // 2 < thumb_tip[0] < rect.posCenter[0] + rect.size[0] // 2) and \
                       (rect.posCenter[1] - rect.size[1] // 2 < thumb_tip[1] < rect.posCenter[1] + rect.size[1] // 2):
                        selected_rect = rect
                        show_palette = True
                        break
